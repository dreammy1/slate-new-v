...
if (!$flash && isset($_GET['gcal_connected']))    $flash = ['type' => 'success', 'msg' => 'Google Calendar connected' . ($_GET['gcal_connected'] !== '1' ? ' (' . $_GET['gcal_connected'] . ')' : '') . '.'];
if (!$flash && isset($_GET['gcal_disconnected'])) $flash = ['type' => 'success', 'msg' => 'Google Calendar disconnected.'];
...
<?php booking_edit_open([
    ...
    <?php if ($editing): ?>
        ...
        ?>
        <?php booking_edit_card_open(['icon' => 'calendar', 'eyebrow' => 'Integrations', 'title' => 'Google Calendar']); ?>
            <?php if (!$gEnabled || !$gConfigured): ?>
                <?php booking_edit_card_note(
                    'Google Calendar sync is off. <a href="' . e(plugin_url('booking', 'admin/settings.php')) . '">Enable it in Booking settings</a> first, then come back here to connect ' . e($editing['name']) . '.'
                ); ?>
            <?php elseif ($gConnected): ?>
                <p class="text-sm" style="margin-bottom:12px;">
                    ...
                    <?php if (!empty($editing['google_connected_email'])): ?>
                        <span class="text-muted"> as <?= e($editing['google_connected_email']) ?></span>
                    <?php endif; ?>
                </p>
                <p class="text-sm text-muted" style="margin-bottom:12px;">
                    New and rescheduled bookings push to this calendar automatically, and events created or moved directly in Google flow back into Slate.
                    <?php if (!empty($editing['google_synced_at'])): ?>Last synced <?= e(date('M j, Y H:i', strtotime($editing['google_synced_at']))) ?>.<?php endif; ?>
                </p>
                <form method="post" action="<?= e(plugin_url('booking', 'admin/gcal-disconnect.php')) ?>" onsubmit="return confirm('Disconnect Google Calendar? Existing synced events on Google will not be deleted.')">
                    <?= csrf_field() ?>
                    <input type="hidden" name="provider_id" value="<?= (int)$editing['id'] ?>">
                    <button type="submit" class="btn btn-sm btn-danger">Disconnect</button>
                </form>
            <?php else: ?>
                <?php booking_edit_card_note("Connect " . e($editing['name']) . "'s Google Calendar so their bookings sync automatically and Slate respects time they've blocked off on their own calendar."); ?>
                <a href="<?= e(plugin_url('booking', 'admin/gcal-connect.php')) ?>?provider_id=<?= (int)$editing['id'] ?>" class="btn btn-sm btn-primary">Connect Google Calendar</a>
            <?php endif; ?>
        <?php booking_edit_card_close(); ?>
    <?php endif; ?>
