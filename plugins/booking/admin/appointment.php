    ...
    if (!csrf_verify()) {
        ...
        if ($action === 'cancel') {
            ...
            if (!empty($res['ok'])) {
                ...
                $flash = ['type' => 'success', 'msg' => 'Booking approved (customer notified).'];
            } else {
                $flash = ['type' => 'error', 'msg' => $res['error'] ?? 'Approve failed.'];
            }
        } elseif ($action === 'decline') {
            $res = BookingAPI::declineAppointment($id, trim((string)($_POST['reason'] ?? '')));
            if (!empty($res['ok'])) {
                $row['status'] = 'cancelled';
                $flash = ['type' => 'success', 'msg' => 'Booking declined (customer notified).'];
            } else {
                $flash = ['type' => 'error', 'msg' => $res['error'] ?? 'Decline failed.'];
            }
        } else {
    ...
    <div>
        ...
        <p class="page-header-sub">
            ...
            <?php if (count($groupSiblings) >= 2): ?>
                <span class="badge badge-accent" style="margin-left:6px;">Group of <?= count($groupSiblings) ?></span>
            <?php endif; ?>
        </p>
    </div>
    <?php if (Auth::can('booking.manage_appointments')): ?>
        <div class="toolbar" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            ...
            <?php if ($row['status'] === 'awaiting_approval'): ?>
                <form method="post" style="display:flex;gap:6px;align-items:center;margin:0;" onsubmit="return confirm('Decline this booking request? The customer will be emailed.')">
                    <?= csrf_field() ?>
                    <input type="hidden" name="_action" value="decline">
                    <input type="text" name="reason" placeholder="Reason (optional, emailed to customer)" style="min-width:220px;">
...
<?php slate_page_layout('with-aside'); ?>
    <div class="page-main">
        <div class="card">
            ...
            $payLabels = ['none'=>'—','pending'=>'Awaiting payment','deposit_paid'=>'Deposit paid','paid'=>'Paid','refunded'=>'Refunded','partially_refunded'=>'Partially refunded','failed'=>'Payment failed'];
            ?>
            <ul class="kv-list">
    ...
    <aside class="page-aside">
        <div class="aside-card">
            ...
            <ul class="kv-list">
                <li class="kv-row"><span class="kv-label">Current</span><span class="kv-value">
                    <?php
                    $bm = ['pending'=>['Pending','warning'],'awaiting_approval'=>['Awaiting approval','warning'],'confirmed'=>['Confirmed','active'],'cancelled'=>['Cancelled','inactive'],'no_show'=>['No show','warning'],'completed'=>['Completed','accent']][$row['status']] ?? ['?',''];
                    ?>
        ...
        <?php if (count($groupSiblings) >= 2): ?>
        <div class="aside-card">
            <div class="aside-card-title">Linked bookings (<?= count($groupSiblings) ?>)</div>
            <ul class="kv-list">
                <?php foreach ($groupSiblings as $sib):
                    $isThis = (int)$sib['id'] === (int)$row['id'];
                    $sbm = ['pending'=>['Pending','warning'],'awaiting_approval'=>['Awaiting approval','warning'],'confirmed'=>['Confirmed','active'],'cancelled'=>['Cancelled','inactive'],'no_show'=>['No show','warning'],'completed'=>['Completed','accent']][$sib['status']] ?? ['?',''];
                ?>
                    <li class="kv-row">
                        <span class="kv-value" style="display:flex;flex-direction:column;gap:2px;<?= $isThis ? 'font-weight:600;' : '' ?>">
                            <?php if ($isThis): ?>
                                <?= e(slate_format_datetime($sib['starts_at'], 'D j M', ' · ')) ?> — this appointment
                            <?php else: ?>
