...
$cust   = Auth::customer();
$cid    = (int) ($cust['id'] ?? 0);
$portal = \Slate\Services\Portal\CustomerPortal::forCustomer($cid);
$row    = $portal->customer();

$widgets = $portal->widgets('home');
$kpis    = $portal->stats();
...
slate_portal_shell_open(['active' => 'home', 'area' => 'home']);
?>
...
<div class="bento">
    <?php if ($widgets): ?>
        <?php foreach ($widgets as $widget): ?>
            <?= is_array($widget) ? ($widget['html'] ?? '') : $widget ?>
        <?php endforeach; ?>
    <?php else: ?>
        <section class="pcard">
