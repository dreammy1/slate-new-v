            ...
            if (!$target) {
                ...
                if ($err) {
                    ...
                    if ($dup) {
                        ...
                        $mode = 'edit';
                        // Reload scoped — see the note in admin/users.php; the same
                        // unscoped-reload-after-scoped-update shape.
                        $editing = Database::row(
                            "SELECT * FROM roles WHERE id = ? AND " . slate_tenant_clause(),
                            [$roleId, slate_tenant_id()]
                        );
                    }
                }
            }
